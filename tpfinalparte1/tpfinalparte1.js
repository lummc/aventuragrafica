...
let escala = 1.5; // Cambia este valor para ajustar el tamaño (1.5, 2, etc.)
const anchoBase = 640;
const altoBase = 480;
...
function setup() {
    createCanvas(anchoBase * escala, altoBase * escala);
...
function touchStarted() {
    ...
    for (let i = 0; i < botonesX.length; i++) {
        if (mx > botonesX[i] && 
            mx < botonesX[i] + botonesAncho[i] &&
            my > botonesY[i] && 
            my < botonesY[i] + botonesAlto[i]) {
...
function dibujarFondo() {
    for (let i = 0; i < altoBase; i++) {
        let c = lerpColor(color(138, 43, 226), color(60, 25, 60), i / altoBase);
        stroke(c);
        line(0, i, anchoBase, i);
    }
}

function dibujarImagen(numImg) {
    image(imagenes[numImg], anchoBase/2 - 190, 20, 380, 270);
    noFill();
    stroke(0, 255, 255);
    strokeWeight(3);
    rect(anchoBase/2 - 190, 20, 380, 270);
}
...
function dibujarLinea() {
    stroke(147, 112, 219);
    strokeWeight(2);
    line(120, 290, anchoBase - 120, 290);
}
